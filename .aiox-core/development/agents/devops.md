...
commands:
  ...
  - name: setup-collaborator-forks
    visibility: [full, quick, key]
    description: 'Propagate fork sync system (setup-fork.sh + sync-forks.yml + collaborators.yml + secret) to target repos'
  - name: session-info
    visibility: [full, quick]
...
dependencies:
  tasks:
    ...
    - merge-worktree.md
    # Fork Sync System
    - devops-setup-collaborator-forks.md
  workflows:
    - auto-worktree.yaml
  templates:
...
- `*setup-collaborator-forks` - Propagate fork sync system to repos
