...
bash "${TEMPLATE_ROOT}/core/bus/send-telegram.sh" "${CHAT_ID}" "${MSG_TEXT}" --topic permissions "${KEYBOARD}" > /dev/null 2>&1 || {
    echo '{"hookSpecificOutput":{"hookEventName":"PermissionRequest","decision":{"behavior":"allow"}}}'
    exit 0
}
...
while [[ $ELAPSED -lt 1800 ]]; do
    if [[ -f "$RESPONSE_FILE" ]]; then
        ...
        if [[ "$DECISION" == "allow" ]]; then
            crm_log "plan_decision" "Plan approved via Telegram" "tool=ExitPlanMode" "decision=allow" "elapsed_s=${ELAPSED}"
            echo '{"hookSpecificOutput":{"hookEventName":"PermissionRequest","decision":{"behavior":"allow"}}}'
        else
            crm_log "plan_decision" "Plan denied via Telegram" "tool=ExitPlanMode" "decision=deny" "elapsed_s=${ELAPSED}"
...
crm_log "plan_decision" "Plan timed out (auto-approved)" "tool=ExitPlanMode" "decision=timeout_allow" "elapsed_s=1800"
bash "${TEMPLATE_ROOT}/core/bus/send-telegram.sh" "${CHAT_ID}" "Plan review TIMED OUT (auto-approved): ${AGENT}" --topic permissions > /dev/null 2>&1 || true
