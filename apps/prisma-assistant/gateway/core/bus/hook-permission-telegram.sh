...
bash "${TEMPLATE_ROOT}/core/bus/send-telegram.sh" "${CHAT_ID}" "${MESSAGE}" --topic permissions "${KEYBOARD}" > /dev/null 2>&1 || {
...
while [[ $ELAPSED -lt $TIMEOUT ]]; do
    if [[ -f "$RESPONSE_FILE" ]]; then
        ...
        if [[ "$DECISION" == "allow" ]]; then
            crm_log "permission_decision" "Approved via Telegram" "tool=${TOOL_NAME}" "decision=allow" "elapsed_s=${ELAPSED}"
            echo '{"hookSpecificOutput":{"hookEventName":"PermissionRequest","decision":{"behavior":"allow"}}}'
        else
            crm_log "permission_decision" "Denied via Telegram" "tool=${TOOL_NAME}" "decision=deny" "elapsed_s=${ELAPSED}"
...
crm_log "permission_decision" "Timed out (auto-denied)" "tool=${TOOL_NAME}" "decision=timeout_deny" "elapsed_s=${TIMEOUT}"
bash "${TEMPLATE_ROOT}/core/bus/send-telegram.sh" "${CHAT_ID}" "Permission request TIMED OUT (auto-denied): ${TOOL_NAME}" --topic permissions > /dev/null 2>&1 || true
